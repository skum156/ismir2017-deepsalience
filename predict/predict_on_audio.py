...
import scipy
import csv
import tensorflow as tf  # Add at top if not present
import pretty_midi
...
def load_model(task):
    ...
    if not os.path.exists(weights_path):
        raise IOError("Cannot find weights path {} for this task.".format(weights_path))
...
def get_singlef0(pitch_activation_mat, freq_grid, time_grid, thresh=0.3, use_neg=True):
...
def save_singlef0_as_midi(times, freqs, midi_path, min_note_len=0.05, midi_instrument=0):
    def hz2midi(f):
        if f <= 0:
            return None
    ...
    instrument = pretty_midi.Instrument(program=midi_instrument)
    prev_note = None
    note_on = None
...
def compute_output(hcqt, time_grid, freq_grid, task, output_format, threshold,
                   use_neg, save_dir, save_name, midi_instrument=0):
    ...
    if output_format == 'singlef0':
        ...
        save_singlef0_as_midi(times, freqs, midi_path, midi_instrument=midi_instrument)
    ...
    if args.task == 'all':
        for task in TASKS:
            ...
            compute_output(
                ...
                args.threshold, args.use_neg, args.save_dir, save_name, args.midi_instrument)
    else:
        compute_output(
...
if __name__ == "__main__":
    ...
    parser.add_argument("-f", "--output_format",
                        ...
                        "multif0 saves a csv of multif0 values. "
    ...
    parser.add_argument("--midi_instrument",
                        type=int,
                        default=0,
                        help="General MIDI program number for output instrument (default 0: Acoustic Grand Piano)")
